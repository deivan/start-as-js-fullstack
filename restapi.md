...
## 6. Контейнеризація додатку за допомогою Docker

Для того, щоб ваш REST API працював однаково стабільно на будь-якому сервері або машині розробника, його найкраще упакувати в Docker-контейнер. Контейнеризація ізолює додаток разом з його залежностями (включаючи правильну версію Node.js).

Для цього потрібно створити два файли в корені вашого проєкту: `Dockerfile` та `.dockerignore`.

### Створення `.dockerignore`

Цей файл працює подібно до `.gitignore`. Він вказує Docker, які файли **не потрібно** копіювати всередину образу (наприклад, локальну папку з модулями, щоб не тягнути зайве сміття і зібрати їх заново в контейнері).
...
Це інструкція для Docker, як саме потрібно зібрати образ вашого додатку. Ми використаємо легковісну версію (alpine) образу Node.js 22.

```dockerfile
# 1. Вказуємо базовий образ з Node.js 22 (alpine - це мінімалістична версія Linux)
FROM node:22-alpine

# 2. Встановлюємо робочу директорію всередині контейнера
WORKDIR /usr/src/app

# 3. Копіюємо файли конфігурації залежностей
# Використовуємо wildcard (*), щоб скопіювати package.json та package-lock.json
COPY package*.json ./

# 4. Встановлюємо залежності (включаючи Express)
# Використовуємо npm ci для більш чистого та швидкого встановлення в CI/CD
RUN npm ci --only=production

# 5. Копіюємо весь інший вихідний код додатку
COPY . .

# 6. Вказуємо порт, який буде прослуховувати контейнер 
# (повинен співпадати з портом у app.listen)
EXPOSE 3000

# 7. Команда, яка буде виконана при старті контейнера
# Рекомендується використовувати 'node app.js' або скрипт з package.json
CMD ["node", "src/app.js"]
```

### Збирання та запуск контейнера

Після створення цих файлів, ви можете зібрати образ та запустити контейнер за допомогою наступних команд у терміналі (перебуваючи в папці проєкту):

**1. Збирання Docker-образу:**
Команда читає `Dockerfile` і створює образ з назвою `my-express-api`.
```bash
docker build -t my-express-api .
```

**2. Запуск Docker-контейнера:**
Запускаємо створений образ. Прапорець `-p 3000:3000` прокидає порт 3000 з вашого локального комп'ютера на порт 3000 всередині контейнера. Прапорець `-d` запускає контейнер у фоновому режимі (detached).
```bash
docker run -p 3000:3000 -d --name express-api-container my-express-api
```

Тепер ваш REST API на базі Node.js 22 та Express.js v5 успішно працює в ізольованому Docker-контейнері і готовий до розгортання на будь-якому хмарному сервері.